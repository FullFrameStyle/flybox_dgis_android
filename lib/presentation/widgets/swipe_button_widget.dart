...
class _SwipeState extends State<SwipeButton> with TickerProviderStateMixin {
                  ...
                  builder: (context, child) {
                    return Container(
                      ...
                      child: Padding(
                        ...
                        child: swiped && widget.thumbEnd != null
                            ? widget.thumbEnd
                            : widget.thumb ??
                                Icon(
                                  Icons.arrow_forward,
    ...
    switch (widget._swipeButtonType) {
      ...
      case _SwipeButtonType.expand:
        if (!swiped && widget.enabled) {
          ...
          if (!preSwiped && expandAnimationController.value >= 0.5) {
            preSwiped = true;
            Timer.periodic(const Duration(milliseconds: 15), (timer) {
              if (expandAnimationController.value < 1) {
                expandAnimationController.value += 0.05;
              } else {
                setState(() {
                  swiped = true;
                  widget.onSwipe?.call();
  ...
  _onHorizontalDragEnd(DragEndDetails details) {
    setState(() {
      ...
      if (!swiped) expandAnimationController.animateTo(0);
    });
